...
To ensure we finish on time, we are splitting the system into 4 distinct domains. **Stick to your domain** to avoid "Merge Conflicts".
...
| **Member C** | **Admin Operations**        | **Management (The "Staff")**   | Building the Admin Dashboard, Charts, and Fleet Management.       |
| **Member D** | **System Integrator**       | **Security & Core Logic**      | Authentication (RBAC), Search Engine, and Data Integrity.         |
...
3.  **One Controller Per Person:** Try not to edit the same Controller at the same time.
    -   Member A works on `PagesController` and Templates.
    -   Member B works on `BookingsController` and `InvoicesController`.
    -   Member C works on `MaintenancesController`, `CarCategoriesController`, and `AdminsController` (Dashboard).
    -   Member D works on `AppController`, `UsersController` (Auth), and `CarsController` (Logic).
...
-   **Key Tasks:**
    ...
    4.  **Invoice View (`Invoices/view`):** Design the HTML invoice to look like a real paper receipt. Use status badges (Green for Paid, Red for Unpaid).
...
-   **Key Tasks:**
    ...
    2.  **Auto-Invoice Logic (`InvoicesController`):**
        -   When a booking is saved, **automatically** create a row in the `invoices` table.
        -   Ensure you fetch deep associations (`Bookings` -> `Users` + `Cars`) so the invoice has customer names.
...
-   **Key Tasks:**
    1.  **Admin Dashboard (`Admins/dashboard`):** Show "Total Bookings", "Pending Reviews", and the Chart.js graph.
    2.  **Fleet Management:** Manage the `CarCategories` and `Maintenances` tables.
    3.  **Management Tables:** Clean up the baked "Index" pages (Users, Bookings). Add the DataTables plugin to them.
...
-   **Key Tasks:**
    1.  **Role-Based Security (`AppController`):**
        -   Since we use a single `users` table, check `$user->role`.
        -   Logic: _If role != 'admin', redirect away from /admin pages._
    2.  **Search Logic (`CarsController`):** Write the backend query that powers Member A's search bar.
        -   _Logic:_ `WHERE car_model LIKE %keyword% OR category_id = filter`.
    3.  **The "Category" Fix (`CarsTable.php`):**
        -   **CRITICAL:** We used `category_id` in the database. You must manually write the association `belongsTo('CarCategories', ['foreignKey' => 'category_id'])` in the Table file, or the system will break.
    4.  **Validation (`UsersTable.php`):**
        -   Ensure `ic_number` is unique and exactly 12 digits.
...
| :---- | :----------------------------------- | :----------------------------------- | :---------------------------------- | :------------------------------------- |
| **1** | Design **Landing Page** & Navbar.    | Setup **CakePdf** engine.            | **Bake** all tables. Setup DB.      | Install **Authentication** (RBAC).     |
| **2** | Build **Car Catalog** (Grid View).   | Build **Booking Form** (Date logic). | Build **Manage Categories**.        | Fix **CarsTable** association logic.   |
| **3** | Design **Invoice** HTML layout.      | Build **Payment Simulation**.        | Build **Admin Dashboard** (Charts). | Write **Search Logic** (for Member A). |
| **4** | Style all forms with **Bootstrap**.  | Connect **Auto-Invoice** trigger.    | Add **DataTables** to lists.        | **Stress Test** (IC/Date validation).  |
...
-   **"I can't add a new Category!"** -> Ask **Member C**.
